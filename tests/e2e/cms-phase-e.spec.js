...
test("E2E Blog Roundtrip: Create Draft, Hide Publicly, Publish & Display Detail", async ({ page }) => {
  ...
  await page.keyboard.type("Rich journal content with artisan details.");
  await page.locator('input[placeholder*="Styling Terracotta"]').click();
  await page.locator('button:has-text("Save Blog Post")').click();
  await expect(page.locator(`tr:has-text("${BLOG_TITLE}")`)).toBeVisible();
...
test("E2E FAQ Roundtrip: Category & Question CRUD, Public Accordion & Deactivation", async ({ page }) => {
  ...
  await page.locator('div[contenteditable="true"]').click();
  await page.keyboard.type("Express delivery takes 3-5 business days across India.");
  await page.locator('input[placeholder*="How long does"]').click();
